...
export default function Home() {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center p-4 sm:p-8 max-w-2xl mx-auto select-none">
      ...
      {user && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 w-full mb-8 animate-in fade-in duration-300">
      ...
      <div className="relative w-full">
        ...
        {!user && (
          <div className="absolute inset-0 flex items-center justify-center p-3 sm:p-6 z-10 animate-in fade-in zoom-in-95 duration-300">
            <div 
              style={{
                backgroundColor: 'rgba(25, 29, 39, 0.88)',
                borderColor: 'var(--surface-border, #262b38)',
              }}
              className="w-full max-w-md p-6 sm:p-8 rounded-3xl border shadow-2xl backdrop-blur-xl flex flex-col items-center text-center gap-4 relative overflow-hidden"
            >
              {/* Subtle emerald glow */}
              <div className="absolute -top-16 left-1/2 -translate-x-1/2 w-48 h-48 bg-[#2ec47d]/20 rounded-full blur-3xl pointer-events-none" />

              <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-[#2ec47d] to-[#10b981] flex items-center justify-center text-white shadow-xl shadow-[#2ec47d]/30">
                <Lock size={24} />
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-[var(--foreground-muted)] mt-1.5 max-w-xs leading-relaxed">
                  Sign in with Google to unlock classic puzzles, timed rush, online 1v1 duels, and rank on the global leaderboard.
                </p>
              </div>

              <button
                onClick={signInWithGoogle}
                className="w-full py-3.5 px-4 rounded-2xl bg-gradient-to-r from-[#2ec47d] to-[#10b981] hover:brightness-105 active:scale-[0.98] text-white font-extrabold text-sm shadow-xl shadow-[#2ec47d]/25 transition-all flex items-center justify-center gap-3 cursor-pointer mt-1"
              >
